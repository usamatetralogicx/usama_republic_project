    ...
    <div class="row mt-2">
        ...
        <div class="col-6">
            ...
            <div class="float-right">
                ...
                @role('supplier')
                <a class="btn btn-primary" href="{{ route('supplier.store.create') }}">Add Store</a>
                @endrole
            ...
            @else
                @foreach($supplier_stores as $store)
                    <tr>
                        ...
                        <td>
                            <a class="pull-right" href data-toggle="modal" data-target="#deleteShopConfirmationModal{{$store->id}}">
                                <span data-toggle="tooltip" title="Click to delete store"><i class="fa fa-trash" style="font-size: 18px"></i></span>
                            </a>
                            @role('supplier')
                            <a class="pull-right mr-1" href="{{route('supplier.store.edit', $store->id) }}">
                                <span data-toggle="tooltip" title="Click to edit store!"><i class="fa fa-edit" style="font-size: 18px"></i></span>
                            </a>


                            @if(isset(\Illuminate\Support\Facades\Auth::user()->markup_setting) && \Illuminate\Support\Facades\Auth::user()->markup_setting->ask_every_time == 0)
                                <a class="pull-right mr-1" href="{{ route('supplier.store.sync.products', $store->id) }}" id="sync-products-with-global-price-settings" >
                            ...
                            @else
                                <a class="pull-right mr-1 text-primary" data-toggle="modal" data-target="#setProfitMarginModal{{$store->id}}" >
                                    <span data-toggle="tooltip" title="Click to fetch all products from your shopify store!"><i class="fa phpdebugbar-fa-refresh" style="font-size: 18px"></i></span>
                                </a>
                            @endif
                        ...
                        <div class="modal" id="setProfitMarginModal{{$store->id}}">
                            <div class="modal-dialog">
                                <div class="modal-content modal-lg">
                                    ...
                                    <div class="modal-body">
                                        <form action="">
                                            ...
                                            <select class="form-control profit-margin-type" name="" id="{{ $store->id }}">
                                                <option value="-1">Select profit margin type</option>
                                                <option value="1">Percentage profit</option>
                                                <option value="2">Fixed profit</option>
                                            </select>
                                            <div hidden id="percentage-profit-{{ $store->id }}" class="form-group mt-1">
                                                <div class="input-group">
                                                    <div class="input-group-prepend">
                                                        <div class="input-group-text">%</div>
                                                    </div>
                                                    <input type="number" id="input-percentage-value-{{$store->id}}" class="form-control" placeholder="Percentage value">
                                                </div>
                                                @if( $store->profit_margin_percentage != null)
                                                    <span>Previously selected profit percentage is: {{ $store->profit_margin_percentage }}</span>
                                                @endif
                                            </div>
                                            <div hidden id="fixed-profit-{{ $store->id }}" class="form-group mt-1">
                                                <div class="input-group">
                                                    <div class="input-group-prepend">
                                                        <div class="input-group-text">$</div>
                                                    </div>
                                                    <input type="number" id="input-fixed-value-{{$store->id}}" class="form-control" placeholder="Fixed value">
                                                </div>
                                                @if( $store->profit_margin_fixed != null)
                                                    <span>Previously selected fixed profit amount is: {{ $store->profit_margin_fixed }}</span>
                                                @endif
                                            </div>
                                    ...
                                    <div class="modal-footer">
                                        ...
                                        <a id="{{ $store->id}}" style="color: white" class="btn btn-primary sync-products">Sync</a>
                                    </div>
                                </div>
                            </div>
                        </div>
    ...
    <script>
        $(function () {
            ...
            $('.sync-products').click(function () {
                ...
                $.ajax({
                    url: '{{ env('APP_URL') }}' + '/supplier/store/set/profit/margin/' + id,
                    type: 'post',
                    data: {
                        _token: "{{csrf_token()}}",
                    ...
                    success: function (success) {
                        if (success['status'] == 200) {
                            if (success['message'] === 'saved') {
                                $.ajax({
                                    url: '{{env('APP_URL')}}' + '/supplier/store/sync/products/' + id,
                                    type: 'get',
                                    success: function (success) {
