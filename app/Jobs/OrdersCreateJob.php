...
use App\RetailerOrderSupplierOrder;
...
class OrdersCreateJob implements ShouldQueue
{
    ...
    {
        Log::critical('Order create web hook: ' . json_encode($this->data));
        $order = $this->data;

        $retailer = User::where('myshopify_domain', $this->shopDomain)->first();

        //its a new product fetch it
        $newOrder = new RetailerOrder();

        $newOrder->retailer_id = $retailer->id;
...
////        //order created, check if this order contain any product imported with our database
//        foreach (json_decode($newOrder->line_items, true) as $item) {
//
//            $checkIfItsLocalProduct = RetailerProduct::where('shopify_product_id', $item['product_id'])
//                ->exists();
//
//            if ($checkIfItsLocalProduct) {
//                $supplier_order = new SupplierOrder();
//
//                $retailer_product = RetailerProduct::where('shopify_product_id', $item['product_id'])->first();
//                $retailer = User::find($retailer_product->retailer_id);
//                $supplier_product = Product::find($retailer_product->product_id);
//                $who_supply_this_product = UserProduct::where('product_id', $supplier_product->id)->first();
//                $supplier = User::find($who_supply_this_product->user_id);
//                $order_details = new RetailerOrderDetails();
//                $order_details->retailer_order_id = $newOrder->id;
//                $order_details->retailer_id = $retailer->id;
//                $order_details->retailer_product_id = $retailer_product->id;
//                $order_details->supplier_id = $supplier->id;
//                $order_details->supplier_product_id = $supplier_product->id;
//                $order_details->save();
//            }
//
//        }
